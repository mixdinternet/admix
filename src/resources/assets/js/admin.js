...
$(function () {
    ...
    $('.jq-select2-load').width('100%').select2({
        language: 'pt-BR'
        , width: '100%'
        , ajax: {
            //url: '/api/cars/brands', //parametro passado por data-ajax--url
            dataType: 'json',
            delay: 250,
    ...
    $('select[name=choose]').on('change', function () {
        $('.jq-choose').addClass('hide');
        $('.jq-choose').find('input').val('');

        var val = $(this).val();
        $('.jq-' + val).removeClass('hide');
