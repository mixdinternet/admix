...
@section('btn-insert')
    @if((!checkRule('admin.roles.create')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.insert', ['route' => route('admin.roles.create')])
    @endif
    @if((!checkRule('admin.roles.trash')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.trash', ['route' => route('admin.roles.trash')])
    @endif
    @if($trash)
        @include('mixdinternet/admix::partials.actions.btn.list', ['route' => route('admin.roles.index')])
    @endif
@endsection

@section('btn-delete-all')
    @if((!checkRule('admin.roles.destroy')) && (!$trash))
        @include('mixdinternet/admix::partials.actions.btn.delete-all', ['route' => route('admin.roles.destroy')])
    @endif
@endsection
        ...
        <table class="table table-striped table-hover table-action jq-table-rocket">
            ...
            @foreach ($roles as $role)
                <tr>
                    @if((!checkRule('admin.roles.destroy')) && (!$trash))
                        <td>
                            @include('mixdinternet/admix::partials.actions.checkbox', ['row' => $role])
                        </td>
                    @endif
                    <td>{{ $role->id }}</td>
                    <td>{{ $role->name }}</td>
                    <td>@include('mixdinternet/admix::partials.label.status', ['status' => $role->status])</td>
                    <td>
                        @if((!checkRule('admin.roles.edit')) && (!$trash))
                            @include('mixdinternet/admix::partials.actions.btn.edit', ['route' => route('admin.roles.edit', $role->id)])
                        @endif
                        @if((!checkRule('admin.roles.destroy')) && (!$trash))
                            @include('mixdinternet/admix::partials.actions.btn.delete', ['route' => route('admin.roles.destroy'), 'id' => $role->id])
                        @endif
                        @if($trash)
                            @include('mixdinternet/admix::partials.actions.btn.restore', ['route' => route('admin.roles.restore', $role->id)])
                        @endif
                    </td>
                </tr>
            @endforeach
...
@section('pagination-showing')
    @include('mixdinternet/admix::partials.pagination-showing', ['model' => $roles])
@endsection